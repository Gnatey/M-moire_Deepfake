...
if page == "Accueil":
    ...
    aware = df[COLUMNS['aware']].value_counts(normalize=True).get('Oui', 0) * 100
    col2.metric("Connaissance des DeepFakes", f"{aware:.1f}%")

    seen = df[COLUMNS['seen']].value_counts(normalize=True).get('Oui', 0) * 100
    col3.metric("Ont vu un DeepFake", f"{seen:.1f}%")

    trust = df[COLUMNS['trust']].value_counts(normalize=True).get('Oui', 0) * 100
    col4.metric("Confiance en ligne", f"{trust:.1f}%")
